...
export const augmentEventChartSpec = (
  ...
  const isBaseVis = get(config, 'isBaseVis', true) as boolean;
  const isEventVis = get(config, 'isEventVis', false) as boolean;
  const isTimelineVis = get(config, 'isTimelineVis', false) as boolean;
